...
class PortalAddReview extends React.Component {
  ...
  handleSubmit(event) {
    ...
    fetch(`http://localhost:3001/api/addreview`, {
        method: 'POST',
        headers: {
  ...
  fetchCourses() {
    let fetchURL = `http://localhost:3001/api/courses?all_courses=1`;
    fetch( fetchURL )
      .then((res) => {
        if(res.status !== 200) {
      ...
      res.json()
        .then((json) => {
          ...
          if (access === true) {
            ...
            var fetchURLUser = `http://localhost:3001/api/user?email=${json.user_email}`;
            fetch( fetchURLUser )
              .then(res => {
                if(res.status !== 200) {
                  console.log('Looks like there was a problem. Status Code: ' +
